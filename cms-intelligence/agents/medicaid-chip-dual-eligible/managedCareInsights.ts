...
async function stateChangeInsight(snapshot: ManagedCareSnapshot, latest: number, prior: number, ctx: AgentContext): Promise<Insight | null> {
  ...
  return validateInsight({
    ...
    businessRelevance: "Comprehensive managed care is how most Medicaid enrollees get coverage, so this is the size of the contracted Medicaid market each state offers health plans.",
    ...
    nextSignal: `CMS's ${latest + 1} release (expected early ${latest + 3}, as ${latest} data arrived in early ${latest + 2}) will show whether the states that fell most kept falling.`,
...
function parentInsight(snapshot: ManagedCareSnapshot, latest: number, prior: number): Insight | null {
  ...
  return validateInsight({
    ...
    businessRelevance: "Which health plan companies hold the most Medicaid managed care membership, and whose footprint grew or shrank; the same companies appear in Medicare Advantage, so this links the two programs.",
