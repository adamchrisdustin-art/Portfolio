...
import { feeScheduleInsights } from "./feeScheduleInsights";
import { hospitalPenaltyInsights } from "./hospitalPenaltyInsights";
...
export const reimbursementPaymentAgent: DomainAgent = {
  ...
  async run(ctx: AgentContext): Promise<Insight[]> {
    const benchmark = await paymentToChargeInsight(ctx);
    return [...(benchmark ? [benchmark] : []), ...(await feeScheduleInsights(ctx)), ...(await hospitalPenaltyInsights(ctx))];
  },
};

async function paymentToChargeInsight(ctx: AgentContext): Promise<Insight | null> {
