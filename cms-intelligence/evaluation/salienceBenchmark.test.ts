...
describe("captureSalienceCalls", () => {
  it("captures the real prompts production sends - one per ranked selection with more candidates than it shows", () => {
    expect(calls).toHaveLength(29); // 16 measured 2026-09-24; on 2026-09-25 +2 physician (provider types, states), +1 NIH institutes, +1 MA share shift, +2 services (categories, codes), +2 Marketplace (benchmark by state, issuers by state - the old 5-state issuer call showed every candidate, so it never counted), +1 Marketplace enrollment by state, +1 Medicaid enrollment by state, +1 Medicaid managed care by state; session B: +1 market growth (hospital-count call replaced by hospital beds and nursing home beds by state), +1 SNF ownership changes by state
