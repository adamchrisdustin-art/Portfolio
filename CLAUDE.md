...
- **Measured exception (2026-09-25):** the Healthcare Intelligence autonomous monthly run routes by live benchmark results, not by this default (`docs/cms-intelligence/MODEL_EVALUATION.md`). Cheap per-agent picks use OpenAI's cheapest capable model (`gpt-6-luna`, per Adam's preference for OpenAI on cheap tasks), and the single monthly executive-analyst call uses `claude-opus-5-5`, which scored highest. That's about $1/year total. Any future escalation still needs measured evidence like this, not model prestige.
